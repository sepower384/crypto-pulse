...
def summarize(lines):
    ...
    if not key or not lines:
        return []
    prompt = (
        "너는 온체인과 커뮤니티를 하루 종일 들여다보는 크립토 디젠 애널리스트다. 아래는 최근 몇 시간 동안 "
        "글로벌 X·레딧·텔레그램, 국내 커뮤니티(디시·코인판)와 속보, 매체 기사, 새 체인, 온체인 핫토큰 데이터다. "
        "한국 투자자에게 '지금 돈과 말이 어디로 몰리는지(내러티브·체인·토큰)'를 3~4개 bullet로 요약해라. "
        "해외와 국내 온도차가 있으면 짚어라. "
