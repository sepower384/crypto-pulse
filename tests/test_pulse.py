...
class TestPipeline(unittest.TestCase):
    ...
    def test_build_message(self):
        ...
        self.assertIn("업계 유명인들이 한 말", text)
        self.assertIn("&lt;security&gt;", text)
        self.assertIn("HYPE(하루 +12%)", text)
        self.assertIn("못 가져온 곳: 유명인 블로그", text)
        self.assertIn("🟪 *jessepollak*", text)
        self.assertIn("룩온체인(고래 지갑 추적)", text)
        self.assertIn("속보·고래 움직임", text)
        self.assertIn("조회 1.2만", text)
